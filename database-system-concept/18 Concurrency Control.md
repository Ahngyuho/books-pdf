...
대학 데이터베이스에서 다음 SQL을 실행하는 트랜잭션 **T30**을 가정합니다:

```sql
SELECT COUNT(*)
FROM instructor
WHERE dept_name = 'Physics';
T30은 instructor 릴레이션 중 Physics 학과에 해당하는 모든 튜플에 접근해야 합니다.
```

다음 SQL 삽입을 수행하는 트랜잭션 T31을 가정합니다:
...
```
스케줄 S에서 T30과 T31 사이에는 다음과 같은 이유로 충돌 가능성이 있습니다.

T30이 COUNT(*) 계산에 T31이 새로 삽입한 튜플을 사용한다면, T30은 T31이 쓴 값을 읽은 것입니다.
→ S와 직렬 동등한 스케줄에서는 T31이 먼저, T30이 나중이어야 합니다.

반대로 T30이 그 새 튜플을 사용하지 않는다면, 직렬 동등한 스케줄에서는 T30이 먼저, T31이 나중이어야 합니다.
